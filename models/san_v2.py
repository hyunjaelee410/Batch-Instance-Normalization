...
class SAN_V2(_BatchNorm):
    def __init__(self, num_features, eps=1e-5, momentum=0.1, affine=True, batch=-1):
        super().__init__(num_features, eps, momentum, affine)
        self.batch = batch

        self.mean_cfc = Parameter(torch.Tensor(num_features, 5))
        self.mean_cfc.data.fill_(0)

        self.std_cfc = Parameter(torch.Tensor(num_features, 5))
    ...
    def get_feat(self, feat):
        ...
        cache = var_i + mean_i ** 2
        var_b = cache.mean(0, keepdim=True) - mean_b ** 2 + self.eps
        std_b = var_b.sqrt()

        feat_ones = torch.ones(B, C, 1).cuda()


        mean_feat = torch.cat((mean_i, std_i, mean_b, std_b, feat_ones), dim=2)
